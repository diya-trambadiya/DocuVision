...
# If no image is provided, a file dialog will open automatically.
...
class DocScanner(object):
    ...
    def scan(self, image_path):
        ...
        # Resize only for display (keep original saved)
        display_height = 700
        h, w = thresh.shape

        if h > display_height:
            scale = display_height / h
            display_image = cv2.resize(thresh, (int(w * scale), display_height))
        else:
            display_image = thresh

        cv2.imshow("Scanned Output", display_image)
        cv2.waitKey(0)
...
if __name__ == "__main__":
    ...
    if im_file_path is None and im_dir is None:
        root = Tk()
        root.withdraw()
